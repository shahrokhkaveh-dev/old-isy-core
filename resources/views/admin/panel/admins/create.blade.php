        ...
        <div class="bg-check-signup ">
            <div class="row m-0 mob-column-reverse">
                <div class="col-12 col-lg-7">
                    <form class="form-signup form-margin" action="{{ route('admin.admin.store') }}" method="post" enctype="multipart/form-data" id="createAdminFrom">
                        @csrf
                        <div class="d-flex justify-content-between">
                            <div>
                                <label>نام: </label>
                                <input type="text" name="fname" class="form-control disabled @error('fname') is-invalid @enderror" placeholder="نام ادمین" value="{{ old('fname') }}" maxlength="40">
                                @error('fname')
                            ...
                            <div>
                                ...
                                <input type="text" name="lname" class="form-control disabled @error('lname') is-invalid @enderror" placeholder="نام خانوادگی مدیر" value="{{ old('lname') }}" maxlength="40">
                                @error('lname')
                                    <span class="text-danger text-small">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="d-flex justify-content-between">
                            <div>
                                ...
                                <input type="text" name="username" class="form-control disabled @error('username') is-invalid @enderror" placeholder="نام کاربری مدیر" value="{{ old('username') }}" maxlength="20">
                                @error('username')
                                    <span class="text-danger text-small">{{ $message }}</span>
                                @enderror
                            </div>
                            <div>
                                ...
                                <select name="role" class="form-control " name="" id="">
                                    <option value="admin" @if(old('role') == 'admin') selected @endif>ادمین</option>
                                </select>
                                @error('role')
                                    <span class="text-danger text-small">{{ $message }}</span>
                                @enderror
