...
@section('content')
    <header class="d-flex" style="justify-content: space-between">
        ...
        <button class="actionBtn">
            <a href="{{ route('panel.advertising.create') }}">افزودن</a>
        </button>
    </header>
            ...
            <table class="table prodductTable">
                <thead>
                    <tr>
                        ...
                        <th style="min-width: 240px">نام خبر</th>
                        <th style="width: 240px">دسته بندی</th>
                        <th style="width: 180px">عملیات</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($advertisings as $key => $product)
                        <tr>
                            <th style="width: 40px">{{ $key + 1 }}</th>
                            <td style="width: 85px"><img style="border-radius: 15px;"
                                    src="{{ asset($product->box_image_path ? $product->box_image_path : 'images/banner1.png') }}"
                                    alt="" class="img-fluid"></td>
                            <td>{{ $product->title }}</td>
                            <td>{{ $product->category ? $product->category->name : '' }}</td>
                            <th>

                                <form action="{{ route('panel.advertising.destroy', $product->id) }}" style="display: inline"
                                    method="post">
                                    @csrf
                                    @method('delete')
                                ...
                                <a href="{{ route('user.advertising.show' , $product->id) }}" target="_blank">
