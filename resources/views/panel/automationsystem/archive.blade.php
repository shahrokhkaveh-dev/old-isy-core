                ...
                <div class="card-header bg-white">
                    <div class="d-flex" style="justify-content: space-between;align-items: center;">
                        ...
                        <div class="input-group input-group-sm" style="width: unset;">
                            <form action="{{ route('automationSystem.inboxpage') }}" method="get" class="d-inline-flex">
                                ...
                                <div class="input-group-append">
                                    <div class="btn btn-primary rounded-0 rounded-start btn-sm">
                        ...
                        <table class="table table-striped table-hover"">
                            ...
                            <tbody>
                                @foreach ($letters as $key => $letter)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ 1000 + $letter->letter_id }}</td>
                                        <td class="text-primary"><a href="{{ route('automationSystem.show' , ['id'=>encrypt($letter->letter_id) , 'type'=>'reciver']) }}">{{ $letter->title }}</a></td>
                                        <td>{{ $letter->name }}</td>
                                        <td>
                                            @if ($letter->seen)
                                                <span class="badge bg-success">مشاهده شده</span>
                                            @else
                                                <span class="badge bg-warning">مشاهده نشده</span>
                                            @endif
                                        </td>
